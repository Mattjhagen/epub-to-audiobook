...
import { parseEpubInBrowser } from './services/epub'
import './App.css'
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    if (file && file.name.endsWith('.epub')) {
      ...
      try {
        ...
        // Electron exposes file.path and IPC parsing. Browser mode parses client-side.
        if (window.ipcRenderer?.invoke && 'path' in file) {
          const filePath = (file as File & { path?: string }).path
          if (!filePath) throw new Error('Unable to resolve file path for Electron parser.')
          parsedChapters = await window.ipcRenderer.invoke('parse-epub', filePath)
        } else {
          parsedChapters = await parseEpubInBrowser(file)
        }
        setChapters(parsedChapters)
      } catch (error) {
        console.error('Error parsing epub:', error)
        alert(`Failed to parse EPUB file: ${error instanceof Error ? error.message : String(error)}`)
      } finally {
        setIsLoading(false)
      }
    }
